...
const TransitionImage = dynamic(() => import("components/media/TransitionImage"));
const BodyComponent = dynamic(() => import("./BodyComponent"));
const TextAndDiagramSlide = dynamic(() => import("components/slides/TextAndDiagramSlide"));
...
const MultipleDiagramSlide = dynamic(() => import("components/slides/MultipleDiagramSlide"));
const Modal = dynamic(() => import("components/layout/Modal"));
const Slide = ({ data, colorTheme, idx, isPreview, downIcon, currentPageIdx }) => {
  const { activeIndex, onHover, onHoverOut } = useDiagramInteraction();
  const { isModalOpen, modalIdx, onClick, onDismiss } = useModals();
  const dynamicImportApplet = data.appletId ? null : null;
  const SLIDE_MAPPER = {
    ...
    [SLIDE_TYPES.TEXT_AND_DIAGRAM]: {
      ...
      props: {
        ...
        diagram: data.transitionImages ? (
          <TransitionImage images={data.transitionImages.map((image) => image.url)} altTexts={data.transitionImages.map((image) => image.altText)} activeIndex={activeIndex} />
        ) : (
          <></>
        ),
        secondaryTitle: data.secondaryTitle,
      ...
      props: {
        ...
        images:
          data.images?.map((image) => ({
            diagram: <TransitionImage images={[image.diagram]} altTexts={[image.altText]} activeIndex={activeIndex} />,
  ...
  if (data.modal) {
    ...
    allModals.forEach((singleModalData, currentModalIdx) => {
      const modalBody = singleModalData.body?.map((item, idx) => <BodyComponent key={idx} item={item} theme={data.theme} isModal />);
      modals.push(
        <Modal
  ...
  if (data.body) {
    data.body.forEach((item, idx) => {
      ...
      if (item.content || item.numberedPoints || item.bulletPoints) {
        const count = ((item.content || item.numberedPoints || item.bulletPoints).match(/%mt/g) || []).length;
        modalCount += count;
      }
    });
  }
  ...
  return (
    // TODO: Fix this
    <div style={{ height: "100%", width: "100%", position: "absolute" }} id={`slide-${idx}`}>
      <SlideComponent.component {...SlideComponent.props} colorTheme={colorTheme} isPreview={isPreview}>
        {modals}
        {children}
