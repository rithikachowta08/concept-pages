...
const Slide = ({ data, colorTheme, idx, isPreview, downIcon, currentPageIdx }) => {
                ...
                case SLATE_CONTENT_TYPES.MATH_EXPRESSION:
                  return {
                    ...
                    children: inline.children.map((inner) => ({
                      ...inner,
                      idx: inner.type === SLATE_CONTENT_TYPES.IMAGE_LINK ? imageIdx++ : undefined,
                    })),
                  };
      ...
      props: {
        ...
        diagram: data.defaultImage ? (
          ...
          <TransitionImage
            ...
            images={transitionImages ? transitionImages.map((image) => image.url) : []}
            altTexts={transitionImages ? transitionImages.map((image) => image.altText) : []}
  ...
  if (data.body && data.body[0] && data.body[0].content) {
    ...
    for (const line of data.body[0].content) {
      const newModalData = line.children.filter((inlineEl) => inlineEl.type === SLATE_CONTENT_TYPES.MODAL_TRIGGER)[0];
      if (newModalData) {
